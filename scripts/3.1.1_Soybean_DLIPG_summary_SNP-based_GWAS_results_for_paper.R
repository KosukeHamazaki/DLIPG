#############################################################################################
######  Title: 3.1.1._Soybean_DLIPG_summary_SNP-based_GWAS_results_for_paper           ######
######  Author: Kosuke Hamazaki (hamazaki@ut-biomet.org)                               ######
######  Affiliation: Lab. of Biometry and Bioinformatics, The University of Tokyo      ######
######  Date: 2022/07/11 (Created), 2023/11/18 (Last Updated)                          ######
#############################################################################################
...
for (nTraitsHere in seq(from = 20, to = 100, by = 20)) {
  ...
  modelNamesOrd <- c("SNP", "M2ALL", "M2C", "M2CJK",
                     "KM", "PC", "DA", "HB", "HBxGB")
    ...
    for (genBackNames in genBackNamesList) {
      ...
      if (!is.null(summaryStatNamesAll)) {
                                          ...
                                          args = lapply(X = summaryStatResListAll,
                                                        FUN = function(summaryStatResListIter) {
                                                          ...
                                                          names(summaryStatResListIterUnlist) <-
                                                            strategyRename2(names(summaryStatResListIterUnlist))
        ...
        if ("China_Japan_Korea" %in% genBackNames) {
          summaryStatMeanMat <- selectRioModel2(summaryStatMeanMat)
        }
        ...
        if ("China_Japan" %in% genBackNames) {
          ...
          summaryStatMeanDf$Strategy[summaryStatMeanDf$Strategy == "C"] <- "M2C"
          summaryStatMeanDf$Strategy[summaryStatMeanDf$Strategy == "J"] <- "M2J"
          summaryStatMeanDf$Strategy[summaryStatMeanDf$Strategy == "K"] <- "M2K"
          summaryStatMeanDf$Strategy[summaryStatMeanDf$Strategy == "C+J+K"] <- "M2CJK"
      ...
      for (summaryStatNamesLogpNow in summaryStatNamesLogpAll) {
        ...
        if ("China_Japan_Korea" %in% genBackNames) {
          strategyNames <- selectRioModelStrategyNames2(strategyNamesRaw)
          summaryStatLogpDf <- summaryStatLogpDf[
            summaryStatLogpDf$Strategy %in% strategyNames,
          ]
        } else {
          strategyNames <- strategyNamesRaw
        }
        ...
        if ("China_Japan" %in% genBackNames) {
          ...
          summaryStatLogpDf$Strategy[summaryStatLogpDf$Strategy == "C"] <- "M2C"
          summaryStatLogpDf$Strategy[summaryStatLogpDf$Strategy == "J"] <- "M2J"
          summaryStatLogpDf$Strategy[summaryStatLogpDf$Strategy == "K"] <- "M2K"
          summaryStatLogpDf$Strategy[summaryStatLogpDf$Strategy == "C+J+K"] <- "M2CJK"
        ...
        colsLogp <- rep(NA, length(strategyNames))
        names(colsLogp) <- strategyNames

        colsLogp[stringr::str_detect(string = strategyNames,
                                     pattern = "DA")] <- "blue"
        colsLogp[stringr::str_detect(string = strategyNames,
                                     pattern = "M")] <- "red"
        colsLogp[stringr::str_detect(string = strategyNames,
      ...
      if ("China_Japan_Korea" %in% genBackNames) {
        recallMeanMat <- selectRioModel2(recallMeanMat)
      }
      ...
      if ("China_Japan" %in% genBackNames) {
        ...
        recallMeanDf$Strategy[recallMeanDf$Strategy == "C"] <- "M2C"
        recallMeanDf$Strategy[recallMeanDf$Strategy == "J"] <- "M2J"
        recallMeanDf$Strategy[recallMeanDf$Strategy == "K"] <- "M2K"
        recallMeanDf$Strategy[recallMeanDf$Strategy == "C+J+K"] <- "M2CJK"
        textXSize <- 22
      } else if ("Japan_Korea" %in% genBackNames) {
        recallMeanDf$Strategy <- stringr::str_remove(
          string = recallMeanDf$Strategy,
  ...
  if (any(nullChrNos != 0)) {
      ...
      args = lapply(X = nullChrResListAll,
                    FUN = function(nullChrResListIter) {
                      ...
                      names(nullChrResListIterUnlist) <-
                        strategyRename2(names(nullChrResListIterUnlist))
    ...
    for (genBackNames in genBackNamesList) {
      ...
      if ("China_Japan" %in% genBackNames) {
        ...
        fdrNullChrMeanDf$Strategy[fdrNullChrMeanDf$Strategy == "C"] <- "M2C"
        fdrNullChrMeanDf$Strategy[fdrNullChrMeanDf$Strategy == "J"] <- "M2J"
        fdrNullChrMeanDf$Strategy[fdrNullChrMeanDf$Strategy == "K"] <- "M2K"
        fdrNullChrMeanDf$Strategy[fdrNullChrMeanDf$Strategy == "C+J+K"] <- "M2CJK"
      ...
      if ("China_Japan_Korea" %in% genBackNames) {
        strategyNames <- selectRioModelStrategyNames2(strategyNamesRaw)
        propMrkUnderThresDf <- propMrkUnderThresDf[propMrkUnderThresDf$Strategy %in% strategyNames, ]
      } else{
        strategyNames <- strategyNamesRaw
      }
      ...
      if ("China_Japan" %in% genBackNames) {
        ...
        propMrkUnderThresDf$Strategy[propMrkUnderThresDf$Strategy == "C"] <- "M2C"
        propMrkUnderThresDf$Strategy[propMrkUnderThresDf$Strategy == "J"] <- "M2J"
        propMrkUnderThresDf$Strategy[propMrkUnderThresDf$Strategy == "K"] <- "M2K"
        propMrkUnderThresDf$Strategy[propMrkUnderThresDf$Strategy == "C+J+K"] <- "M2CJK"
      } else if ("Japan_Korea" %in% genBackNames) {
        propMrkUnderThresDf$Strategy <- stringr::str_remove(
          string = propMrkUnderThresDf$Strategy,
          pattern = "JK."
      ...
      colsProp <- rep(NA, length(strategyNames))
      names(colsProp) <- strategyNames

      colsProp[stringr::str_detect(string = strategyNames,
                                   pattern = "DA")] <- "blue"
      colsProp[stringr::str_detect(string = strategyNames,
                                   pattern = "M")] <- "red"
      colsProp[stringr::str_detect(string = strategyNames,
      ...
      for (strategyNo in 1:nrow(scoresNullChrList[[1]])) {
        ...
        if ("China_Japan" %in% genBackNames) {
          ...
          strategyNameNow[strategyNameNow == "C"] <- "M2C"
          strategyNameNow[strategyNameNow == "J"] <- "M2J"
          strategyNameNow[strategyNameNow == "K"] <- "M2K"
          strategyNameNow[strategyNameNow == "C+J+K"] <- "M2CJK"
